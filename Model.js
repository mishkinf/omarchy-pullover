...
// ---------------------------------------------------------------------------
// handlers: which per-sender actions a message offers
//
// This mirrors handler_for/handler_actions in bin/pullover, and deliberately
// only decides what the panel SHOWS. The daemon re-runs the identical test
// before it executes anything, so a false positive here costs a hint that then
// refuses -- never a command that should not have run.

// A key the panel already spends: 'a' acknowledges, 'd' dismisses, 'c' clears,
// j/k move. A config file cannot take one of these.
var RESERVED_ACTION_KEYS = "acdjk"

function parseHandlers(raw) {
  if (!raw) return []
  try {
    var parsed = JSON.parse(raw)
    if (!parsed || !Array.isArray(parsed.handlers)) return []
    var out = []
    for (var i = 0; i < parsed.handlers.length; i++) {
      var handler = parsed.handlers[i]
      if (handler && typeof handler === "object") out.push(handler)
    }
    return out
  } catch (e) {
    // A broken config file must not take the panel down with it.
    return []
  }
}

// Exact, case-folded equality on the app name -- never a substring and never a
// pattern. See the long note on handler_for() in bin/pullover for why that
// distinction is the whole safety argument.
function handlerFor(message, handlers) {
  if (!message || !handlers) return null
  var app = String(message.app || "").trim().toLowerCase()
  if (app === "") return null
  for (var i = 0; i < handlers.length; i++) {
    var match = handlers[i].match
    if (!match || typeof match.app !== "string") continue
    if (match.app.trim().toLowerCase() === app) return handlers[i]
  }
  return null
}

function handlerActions(handler) {
  if (!handler || !Array.isArray(handler.actions)) return []
  var out = []
  var seen = {}
  for (var i = 0; i < handler.actions.length; i++) {
    var action = handler.actions[i]
    if (!action || typeof action !== "object") continue
    var key = String(action.key || "").trim().toLowerCase()
    if (key.length !== 1 || !/[a-z0-9]/.test(key)) continue
    if (RESERVED_ACTION_KEYS.indexOf(key) !== -1 || seen[key]) continue
    if (!Array.isArray(action.run) || action.run.length === 0) continue
    seen[key] = true
    out.push({ key: key, label: String(action.label || key) })
  }
  return out
}

function actionsFor(message, handlers) {
  return handlerActions(handlerFor(message, handlers))
}

// "i to investigate" -- the same shape as the emergency hint line.
function actionHint(actions) {
  if (!actions || actions.length === 0) return ""
  var parts = []
  for (var i = 0; i < actions.length; i++) {
    parts.push(actions[i].key + " to " + actions[i].label.toLowerCase())
  }
  return parts.join(" · ")
}

// String comparison, like everything else that touches an id here: these are
// 19 digits and Number() rounds them.
function messageById(messages, idStr) {
  if (!messages || !idStr) return null
  for (var i = 0; i < messages.length; i++) {
    if (messages[i].idStr === idStr) return messages[i]
  }
  return null
}
